...
- `docs/` — planning artifacts (brainstorms, ideation, notes, requests/inbox, reviews, spikes, onboarding); requests/completed/ archives closed work
...
- `.claude/` — harness config (`settings.json`, `CODEMAP.md` — tracked; commit after `/codemap` to reset the git-history freshness gate, despite a vestigial `.gitignore` entry); `.claude/agents/` + `.claude/skills/` are sync output of `cas integrate`; `.claude/workflows/` holds the cas-code-review Workflow scripts (synced from `cas-cli/src/builtins/workflows/`)
...
- `requests/` — cross-team BUG/FEATURE inboxes (`README.md` documents the protocol); active: `BUG-team-push-not-chunked.md`, `FEATURE-nuxt-playwright-skill.md`, `team-memories-filter-policy.md`, `RESOLVED-api-me-deploy-failed-type-check.md`, `SHIPPED-user-team-membership-endpoint.md`, `RESPONSE-user-team-membership-endpoint.md`
- `notes/` — working notes: `2026-06-02-cc160-hook-surface.md` (CC hook EPIC) + `claude-code-changelog-diary.md` (rolling Claude Code release → CAS-response ledger)
