...
// Dead code flag removed - all items now used
...
impl MetricsStore {
    ...
    pub fn record_search(&self, event: &SearchEvent) -> Result<(), MemError> {
        self.db.execute(
            r#"
    ...
    pub fn record_feedback(&self, feedback: &ResultFeedback) -> Result<(), MemError> {
        self.db.execute(
            r#"
    ...
    pub fn get_method_metrics(&self) -> Result<HashMap<SearchMethod, MethodMetrics>, MemError> {
    ...
    pub fn get_recent_events(&self, limit: usize) -> Result<Vec<SearchEvent>, MemError> {
        let mut stmt = self.db.prepare(
            r#"
    ...
    ) -> Result<MethodComparison, MemError> {
...
#[cfg(test)]
mod tests {
    use crate::hybrid_search::metrics::*;
