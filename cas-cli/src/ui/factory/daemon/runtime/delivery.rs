...
/// Decide the queue bookkeeping for one delivery attempt (cas-6257).
///
...
mod tests {
    ...
    #[test]
    fn queued_delivery_failure_to_unknown_but_current_target_retries() {
        // Pane not found yet, but the target is still a current session member
        // (mid-spawn) → retry, not abandon.
        assert_eq!(
    ...
    #[test]
    fn queued_delivery_failure_to_gone_target_is_abandoned() {
        // Pane not found AND not a current worker/supervisor → abandon so a
        // stale cross-session row cannot wedge the queue forever.
        assert_eq!(
