...
use crate::harness_policy::{
    supervisor_verification_tool, verification_required_for_task_type, worker_coordination_tool,
};
use crate::store::{
        ...
        match self.check_pending_verification(&agent_id, close_task_id)? {
            Some((task_id, task_title)) => {
                ...
                let guidance = if is_factory_worker {
                    ...
                    let sup_ver_tool = supervisor_verification_tool();
                    format!(
                        "Message supervisor via: \
                         ...
                         Please run verification (task-verifier or direct {sup_ver_tool}) for task {task_id} \
