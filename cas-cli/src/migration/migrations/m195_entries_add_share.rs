//! Migration: Add `share` column to the `entries` table.
//!
...
pub const MIGRATION: Migration = Migration {
    ...
    description: "Add share TEXT column to entries table for older DBs that predate the column",
    up: &["ALTER TABLE entries ADD COLUMN share TEXT"],
    detect: Some(
        "SELECT CASE WHEN EXISTS (SELECT 1 FROM pragma_table_info('entries') WHERE name = 'share') THEN 1 ELSE 0 END",
    ),
};
...
mod tests {
    ...
    fn create_entries_without_share(conn: &Connection) {
        conn.execute_batch(
    ...
    fn has_share_column(conn: &Connection) -> bool {
        let count: i64 = conn
            .query_row(
                "SELECT COUNT(*) FROM pragma_table_info('entries') WHERE name = 'share'",
                [],
                |row| row.get(0),
            )
            .unwrap();
    ...
    fn fresh_db_gets_share_column() {
        ...
        create_entries_without_share(&conn);
    ...
    fn idempotent_when_share_already_present() {
        ...
        .unwrap();
        conn.execute(
            "INSERT INTO entries (id, type, content, share) VALUES ('e1', 'learning', 'hello', 'public')",
            [],
        )
        .unwrap();
        ...
        let share: Option<String> = conn
            .query_row("SELECT share FROM entries WHERE id = 'e1'", [], |row| {
                row.get(0)
            })
            .unwrap();
        assert_eq!(share.as_deref(), Some("public"));
    }
}
