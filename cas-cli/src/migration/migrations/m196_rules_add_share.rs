//! Migration: Add `share` column to the `rules` table.
//!
...
pub const MIGRATION: Migration = Migration {
    ...
    description: "Add share TEXT column to rules table for older DBs that predate the column",
    up: &["ALTER TABLE rules ADD COLUMN share TEXT"],
    detect: Some(
        "SELECT CASE WHEN EXISTS (SELECT 1 FROM pragma_table_info('rules') WHERE name = 'share') THEN 1 ELSE 0 END",
    ),
};
...
mod tests {
    ...
    fn create_rules_without_share(conn: &Connection) {
        conn.execute_batch(
            "CREATE TABLE rules (
                id TEXT PRIMARY KEY,
    ...
    fn has_share_column(conn: &Connection) -> bool {
        let count: i64 = conn
            .query_row(
                "SELECT COUNT(*) FROM pragma_table_info('rules') WHERE name = 'share'",
                [],
                |row| row.get(0),
            )
            .unwrap();
    ...
    fn fresh_db_gets_share_column() {
        ...
        create_rules_without_share(&conn);
    ...
    fn idempotent_when_share_already_present() {
        ...
        .unwrap();
        conn.execute(
            "INSERT INTO rules (id, created, content, share) VALUES ('r1', '2024-01-01', 'always check', 'team')",
            [],
        )
        .unwrap();
        ...
        let share: Option<String> = conn
            .query_row("SELECT share FROM rules WHERE id = 'r1'", [], |row| {
                row.get(0)
            })
            .unwrap();
