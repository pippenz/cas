//! Migration: Add `share` column to the `skills` table.
//!
...
pub const MIGRATION: Migration = Migration {
    ...
    description: "Add share TEXT column to skills table for older DBs that predate the column",
    up: &["ALTER TABLE skills ADD COLUMN share TEXT"],
    detect: Some(
        "SELECT CASE WHEN EXISTS (SELECT 1 FROM pragma_table_info('skills') WHERE name = 'share') THEN 1 ELSE 0 END",
    ),
};
...
mod tests {
    ...
    fn create_skills_without_share(conn: &Connection) {
        conn.execute_batch(
            "CREATE TABLE skills (
                id TEXT PRIMARY KEY,
                name TEXT NOT NULL,
    ...
    fn has_share_column(conn: &Connection) -> bool {
        let count: i64 = conn
            .query_row(
                "SELECT COUNT(*) FROM pragma_table_info('skills') WHERE name = 'share'",
                [],
                |row| row.get(0),
            )
            .unwrap();
    ...
    fn fresh_db_gets_share_column() {
        ...
        create_skills_without_share(&conn);
    ...
    fn idempotent_when_share_already_present() {
        ...
        .unwrap();
        conn.execute(
            "INSERT INTO skills (id, name, content, share) VALUES ('s1', 'deploy', 'run deploy.sh', 'org')",
            [],
        )
        .unwrap();
        ...
        let share: Option<String> = conn
            .query_row("SELECT share FROM skills WHERE id = 's1'", [], |row| {
                row.get(0)
            })
            .unwrap();
        assert_eq!(share.as_deref(), Some("org"));
    }
}
