...
- **Never monitor, poll, or sleep.** Push-based: after assign, wait for messages. MERGE REQUIRED/`awaiting_merge` is an injected drain (merge factory/*→epic, re-close) that outranks free-form chat — not polling; see [workflow.md](cas-supervisor/references/workflow.md).
...
- **Tier every spawn — never fleet-default.** Explicit `model=`/`effort=` every spawn (light / standard floor / heavy / frontier). Deep-dive: [model-selection.md](cas-supervisor/references/model-selection.md).
