...
- **Never spawn raw `Agent(isolation: "worktree")` subagents.** Use `cas__coordination action=spawn_workers count=N isolate=true`; CAS-managed worktrees are tracked, leased, merged, and cleaned up. Non-isolation `Agent` calls for read-only research/review remain fine.
