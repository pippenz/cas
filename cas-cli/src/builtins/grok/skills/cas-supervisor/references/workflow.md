...
1. Spawn workers:
   ```
   cas__coordination action=spawn_workers count=N isolate=true cli=grok model=grok-4.5 effort=medium
   ```
   ...
   **Tiered mix example** — standard floor + light Composer + heavy Grok:
   ...
   cas__coordination action=spawn_workers count=2 cli=grok model=grok-4.5 effort=medium isolate=true

   # Light / flash (Composer is a Grok model id — not cli=cursor)
   cas__coordination action=spawn_workers count=1 cli=grok model=grok-composer-2.5-fast worker_names="lt-ada" isolate=true

   # Heavy (Grok 4.5 high)
   cas__coordination action=spawn_workers count=1 cli=grok model=grok-4.5 effort=high worker_names="hv-ada" isolate=true
   ```
...
5. Search for relevant context and send assignment message:
   ```
   cas__coordination action=message target=<worker> message="Task <id>: <description>. Context: <findings>. Run cas__task action=mine to see your tasks."
   ```
...
6. Message other active workers to sync onto the **local** branch (not `origin/`):
   ```
   cas__coordination action=message target=<other-worker> message="Branch updated after cherry-pick. Sync: git stash && git rebase <base-branch> && git stash pop"
   ```
