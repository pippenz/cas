...
description: Pre-close discipline — run the proof command FRESH, capture its exit code + tail output, then claim done. Use right before `cas__task action=close` to kill the "narrate done before proving it" failure mode. Trigger whenever you are about to assert tests pass, the build is clean, the script works, the bug is fixed, or the AC is satisfied. If you cannot name a proof command, you cannot claim done.
managed_by: cas
---
