...
1. Spawn workers:
   ...
   **Tiered mix example** — Codex-first standard floor + light + heavy:
   ...
   mcp__cas__coordination action=spawn_workers count=1 cli=codex model=gpt-5.5 effort=low worker_names="lt-ada" isolate=true

   # Heavy (Codex gpt-5.5 high); frontier is model=gpt-5.6-sol
   mcp__cas__coordination action=spawn_workers count=1 cli=codex model=gpt-5.5 effort=high worker_names="hv-ada" isolate=true
   ```
