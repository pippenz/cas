...
| `model` | string | Explicit model name. Codex (default matrix): plain `gpt-5.5` or `gpt-5.6-sol` (no `-codex` suffix; bare `gpt-5.6` is invalid). Claude: aliases `sonnet`/`opus`/`haiku` (or full id). Grok: `grok-4.5` or `grok-composer-2.5-fast` (from `grok models`). Passed as `-m`/`--model`. If omitted, resolves through factory config, then backend stock fallback. |
