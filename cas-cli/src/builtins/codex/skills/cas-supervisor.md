...
- **Never spawn raw `Agent(isolation: "worktree")` subagents.** The built-in Agent tool with `isolation: "worktree"` creates `.claude/worktrees/agent-*` directories that live outside CAS factory accounting — no task row, no agent row, no merge-pipeline visibility, and no lease or heartbeat tracking. Use `mcp__cs__coordination action=spawn_workers count=N isolate=true` instead; workers spawned through the coordination tool get a tracked worktree, a lease, and a path into the merge pipeline when their task closes. Non-isolation `Agent` calls for research, review, or other read-only sub-agent work remain fine — the prohibition is specific to the `isolation: "worktree"` variant.
- **Never implement tasks yourself. Delegate ALL non-trivial work to workers.** "Work" here is not just coding. It explicitly includes: reports, analyses, investigations, multi-file edits, runbook updates, architectural docs, design write-ups, and any non-trivial writing. If the answer is more than a few sentences or touches more than one file, it is a task — create it and assign it. Trivial exceptions you may do inline: read-only Q&A, a single `mcp__cs__memory` save, a single-line config change, status updates to the user. Everything else gets a task. **Self-check before every tool call:** Am I about to READ something (acceptable — supervisors read freely) or WRITE/CREATE something (should be a worker task)? If the tool call would produce a file edit, a new file, or a code change, stop and create a task instead.
...
3. **Create EPIC** — `mcp__cs__task action=create task_type=epic title="..." description="..."`
4. **Spawn workers** — `mcp__cs__coordination action=spawn_workers count=N isolate=true`
