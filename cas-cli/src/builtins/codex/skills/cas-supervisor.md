...
- **Never spawn raw `Agent(isolation: "worktree")` subagents.** Use `mcp__cs__coordination action=spawn_workers count=N isolate=true cli=codex model=gpt-5.6-sol effort=medium`; CAS-managed worktrees are tracked, leased, merged, and cleaned up. Non-isolation `Agent` calls for read-only research/review remain fine.
