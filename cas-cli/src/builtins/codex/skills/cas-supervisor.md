...
- **Never use AskUserQuestion for agent communication.** It is only for the **human** user and pauses the system. Use `mcp__cs__coordination action=message` for workers.
- **Never spawn raw `Agent(isolation: "worktree")` subagents.** Use `mcp__cs__coordination action=spawn_workers count=N isolate=true`; CAS-managed worktrees are tracked, leased, merged, and cleaned up. Non-isolation `Agent` calls for read-only research/review remain fine.
