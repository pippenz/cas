...
pub(super) fn register_llm(registry: &mut ConfigRegistry) {
    ...
    registry.register(ConfigMeta {
        ...
        section: "llm",
        name: "Worker Model",
        description: "Override model for worker agents. Fallback chain: [llm.worker.model] → [llm.model] → stock worker default (claude-sonnet-5).",
        value_type: ConfigType::String,
        default: "(inherit)",
        constraint: Constraint::None,
        advanced: false,
    ...
    registry.register(ConfigMeta {
        ...
        description: "Override reasoning effort for worker agents. Fallback chain: [llm.worker.reasoning_effort] → [llm.reasoning_effort] → stock worker default (xhigh).",
        value_type: ConfigType::String,
        default: "(inherit)",
        constraint: Constraint::OneOf(vec!["low".to_string(), "medium".to_string(), "high".to_string(), "xhigh".to_string(), "max".to_string()]),
        advanced: true,
