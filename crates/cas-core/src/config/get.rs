use crate::config::{CloudSyncConfig, Config, CoordinationMode, DevConfig, EmbeddingConfig, HookConfig, McpConfig, NotificationConfig, TasksConfig};
...
impl Config {
    ...
    pub fn get(&self, key: &str) -> Option<String> {
        ...
        match key {
            ...
            "hooks.plan_mode.show_dependencies" => Some(hooks.plan_mode.show_dependencies.to_string()),
            ...
            // MCP section
            "mcp.enabled" => Some(mcp.enabled.to_string()),
            // Dev section
            ...
            // Embedding section
            "embedding.enabled" => Some(embedding.enabled.to_string()),
            "embedding.model" => Some(embedding.model.clone()),
            "embedding.reranker" => Some(embedding.reranker.clone().unwrap_or_default()),
            "embedding.batch_size" => Some(embedding.batch_size.to_string()),
            "embedding.max_per_run" => Some(embedding.max_per_run.to_string()),
            "embedding.interval_secs" => Some(embedding.interval_secs.to_string()),
            ...
            "notifications.display_duration_secs" => Some(notifications.display_duration_secs.to_string()),
            ...
            "notifications.entries.on_updated" => Some(notifications.entries.on_updated.to_string()),
            "notifications.entries.on_deleted" => Some(notifications.entries.on_deleted.to_string()),
            ...
            "notifications.skills.on_disabled" => Some(notifications.skills.on_disabled.to_string()),
            // Agent section
            "agent.name" => Some(self.agent.clone().unwrap_or_default().name),
            "agent.max_concurrent_tasks" => Some(self.agent.clone().unwrap_or_default().max_concurrent_tasks.to_string()),
            "agent.capabilities" => Some(self.agent.clone().unwrap_or_default().capabilities.join(",")),
            ...
            // Lease section
            "lease.default_duration_mins" => Some(self.lease.clone().unwrap_or_default().default_duration_mins.to_string()),
            "lease.max_duration_mins" => Some(self.lease.clone().unwrap_or_default().max_duration_mins.to_string()),
            "lease.heartbeat_interval_secs" => Some(self.lease.clone().unwrap_or_default().heartbeat_interval_secs.to_string()),
            "lease.expiry_grace_secs" => Some(self.lease.clone().unwrap_or_default().expiry_grace_secs.to_string()),
            // Verification section
            "verification.enabled" => Some(self.verification.clone().unwrap_or_default().enabled.to_string()),
            "verification.model" => Some(self.verification.clone().unwrap_or_default().model),
            "verification.force_bypass_allowed" => Some(self.verification.clone().unwrap_or_default().force_bypass_allowed.to_string()),
            "verification.timeout_secs" => Some(self.verification.clone().unwrap_or_default().timeout_secs.to_string()),
            "verification.patterns.todo_comments" => Some(self.verification.clone().unwrap_or_default().patterns.todo_comments.to_string()),
            "verification.patterns.temporal_shortcuts" => Some(self.verification.clone().unwrap_or_default().patterns.temporal_shortcuts.to_string()),
            "verification.patterns.stub_implementations" => Some(self.verification.clone().unwrap_or_default().patterns.stub_implementations.to_string()),
            "verification.patterns.empty_error_handlers" => Some(self.verification.clone().unwrap_or_default().patterns.empty_error_handlers.to_string()),
            "verification.patterns.custom" => Some(self.verification.clone().unwrap_or_default().patterns.custom.join(",")),
            // Worktrees section
            "worktrees.enabled" => Some(self.worktrees.clone().unwrap_or_default().enabled.to_string()),
            "worktrees.base_path" => Some(self.worktrees.clone().unwrap_or_default().base_path),
            "worktrees.branch_prefix" => Some(self.worktrees.clone().unwrap_or_default().branch_prefix),
            "worktrees.auto_merge" => Some(self.worktrees.clone().unwrap_or_default().auto_merge.to_string()),
            "worktrees.cleanup_on_close" => Some(self.worktrees.clone().unwrap_or_default().cleanup_on_close.to_string()),
            "worktrees.promote_entries_on_merge" => Some(self.worktrees.clone().unwrap_or_default().promote_entries_on_merge.to_string()),
            // Factory section
            "factory.warn_stale_assignment" => Some(self.factory.clone().unwrap_or_default().warn_stale_assignment.to_string()),
            "factory.block_stale_assignment" => Some(self.factory.clone().unwrap_or_default().block_stale_assignment.to_string()),
            "factory.stale_threshold_commits" => Some(self.factory.clone().unwrap_or_default().stale_threshold_commits.to_string()),
            _ => None,
        }
    }
}
